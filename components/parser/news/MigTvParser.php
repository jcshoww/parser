...
/**
 * News parser from site https://tvmig.ru/
 * @author jcshow
 */
class MigTvParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://tvmig.ru';
    ...
    public static function getNewsData(int $limit = 100): array
    {
        ...
        while (self::$parsedCount < $limit) {
            /** Get news list */
            $curl = Helper::getCurl();
            $newsList = $curl->get(static::SITE_URL . "/news/?PAGEN_2=$pageNum");
            ...
            $news = $newsListCrawler->filterXPath('//div[contains(@class,"container-column")]//div[contains(@class,"news--item")]');
    ...
    public static function getPostDetail(DOMElement $item): NewsPost
    {
        ...
        $titleBlock = $itemCrawler->filterXPath('//h4[contains(@itemprop,"headline")]//a')->getNode(0);
        ...
        /** Get item datetime */
        $createdAtBlock = $itemCrawler->filterXPath('//div[contains(@class,"post--info")]//time')->getNode(0);
        $stringToDate = self::convertDateToEng($createdAtBlock->textContent);
        $createdAt = new DateTime($stringToDate . ' ' . date('H:i:s'));
        $createdAt->setTimezone(new DateTimeZone('UTC'));
        $createdAt = $createdAt->format('c');

        //Get image if exists
        $picture = null;
        $imageBlock = $itemCrawler->filterXPath('
            //div[contains(@class,"pull-left")]
            //a
            //img[1]
        ')->getNode(0);
        if (! empty($imageBlock) === true) {
            $picture = self::cleanUrl($imageBlock->getAttribute('data-src'));
        ...
        self::removeNodes($crawler, '
            //comment() | //br | //hr | //script | //style | //head | //link | //meta | //form
        ');
        ...
        foreach ($nodeIterator->getRecursiveIterator() as $node) {
            ...
            if (self::hasActualText($text) === true) {
                $description[] = $text;
