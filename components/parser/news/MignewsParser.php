...
/**
 * News parser from site http://mignews.ru/
 * @author jcshow
 */
class MignewsParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'http://mignews.ru';
    ...
    public static function getNewsData(): array
    {
        ...
        /** Get RSS news list */
        $curl = Helper::getCurl();
        $newsList = $curl->get(static::SITE_URL . "/export/yandex_rss2.html");
        if (! $newsList) {
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        self::removeNodes($crawler, '//div[contains(@id,"content")]//div[contains(@id,"leftc")]//h1');
        self::removeNodes($crawler, '//div[contains(@id,"content")]//div[contains(@id,"leftc")]//*[contains(@class,"padbig")]/following-sibling::*');
        self::removeNodes($crawler, '//div[contains(@id,"content")]//div[contains(@id,"leftc")]//*[contains(@class,"padbig")]');

        /** Get item datetime */
        $dateTimeBlock = $crawler->filterXPath('
            //div[contains(@id,"content")]
            //div[contains(@id,"leftc")]
            //span[contains(@class,"txtm")]
        ')->getNode(0);
        $dateTimeString = preg_replace('/(.*)(\:\d{2})(\s.*)/', '$1$2', $dateTimeBlock->textContent);
        $dateTimeString = preg_replace('/\s/', '.' . date('Y') . ' ', $dateTimeString);
        $createdAt = new DateTime($dateTimeString, new DateTimeZone('Europe/Moscow'));
        $createdAt->setTimezone(new DateTimeZone('UTC'));
        $createdAt = $createdAt->format('c');
        $dateTimeBlock->parentNode->removeChild($dateTimeBlock);
        ...
        $detailPage = $crawler->filterXPath('//div[contains(@id,"content")]//div[contains(@id,"leftc")]')->getNode(0);
