...
class VyborNaroda2017Parser implements ParserInterface
{
    ...
    public static function getPostDetail(DOMElement $item)
    {
        ...
        $detailPage = $crawler->filter('div.full-text');

        /** @var DOMNode $node */
        foreach ($detailPage->parents()->first()->getNode(0)->childNodes as $node) {
            if(!in_array($node->nodeName, ["#text", "a"])){
                continue;
            }
    ...
    public static function parseNode(NewsPost $post, DOMNode $node, bool $skipText = false): void
    {
        ...
        foreach (self::$parsedEntities as $entity) {
            $innerNode = new Crawler($node);
            if ($innerNode->filter($entity)->count() > 0) {
                $needRecursive = true;
                break;
            }
        }
