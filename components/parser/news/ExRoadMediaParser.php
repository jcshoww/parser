...
/**
 * News parser from site https://ex-roadmedia.ru/
 * @author jcshow
 */
class ExRoadMediaParser implements ParserInterface
{
    public const USER_ID = 2;

    public const FEED_ID = 2;

    public const SITE_URL = 'https://ex-roadmedia.ru';
    ...
    public static function getNewsData(): array
    {
        ...
        /** Get RSS news list */
        $curl = Helper::getCurl();
        $newsList = $curl->get(static::SITE_URL . "/news?format=feed&type=rss");
        if (! $newsList) {
    ...
    public static function getPostDetail(DOMElement $item): NewsPost
    {
        ...
        /** Description data parser creation */
        $descriptionHtml = self::cleanText($itemCrawler->filterXPath('//description')->text());
